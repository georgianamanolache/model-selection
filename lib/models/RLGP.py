...
class RLGP(MetaGP):
    ...
    def compute_weights(self, X, y):
        ...
        sampled_rankings_m = np.array(sampled_rankings_m)  # for target model

        sampled_rankings.append(sampled_rankings_m)
        sampled_rankings = np.array(sampled_rankings)  # for base models

        # Compute scores
        weights = np.zeros(len(self.base_models) + 1)  # weights
        scores = np.zeros((N_SAMPLES, len(self.base_models) + 1))

        for i in range(len(self.base_models) + 1):
            for sample_idx in range(N_SAMPLES):

                RL = (sampled_rankings[i][sample_idx] ^ true_rankings).astype(int)

                scores[sample_idx][i] = np.sum(RL)
        ...
        # Compute weights
        # The smaller the score the better, as difference is smaller
        for sample_idx in range(N_SAMPLES):
